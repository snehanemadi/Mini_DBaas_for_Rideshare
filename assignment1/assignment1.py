...
def create_ride():
	...
	#existing_username = requests.post(url=urlr, json={"table":"User","col":1,"username":username})
	allusers = requests.get(url = 'http://0.0.0.0/api/v1/users')
	#print(allusers)
	allusers1 = allusers.json()
	#print(allusers1)
	
	#allusers2=allusers1.values()
	#print(allusers2)
...
def join_ride(ride_id):
	...
	allusers1 = allusers.json()
	#print(allusers1) #prints {key:["",""]}
	
	#allusers2=allusers1.values()
	#print(allusers2) #prints dict_values([["",""]])

	present=False
	for name in allusers1:
		#print(name) #prints ["",""]
		#for name1 in name:
			#print(name1) #prints "name"
		if(name==username):
			present=True
