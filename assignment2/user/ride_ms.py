...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(basedir, 'ride.sqlite')
db = SQLAlchemy(app)
ma = Marshmallow(app)
...
def create_ride():
	...
	urlr = 'http://0.0.0.0:8000/api/v1/db/read'
	
	#existing_username = requests.post(url=urlr, json={"table":"User","col":1,"username":username})
	allusers = requests.get(url = 'http://172.17.0.1:8080/api/v1/users')
